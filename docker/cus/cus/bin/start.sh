...
if [ -n "$BITS" ]; then
    ...
    if [ "$SERVICE_NAME"x = "ecnetwork-controller"x ];then
        ...
        JAVA_SERVICE_MEN=" -server -Xmx256m -Xms256m  -XX:MaxMetaspaceSize=128m -Xss256k "
    fi
...
result=$(echo $BIN_DIR | grep "cus\|ecnetwork-shop")
if [ "$result" != "" ];
then
...
echo "java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_GC_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.alibaba.dubbo.container.Main $SPRINGBOOT_OPTS >/dev/null 2>$ERROR_OUT_FILE" >> /root/b.txt
